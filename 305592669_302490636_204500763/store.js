...
app.get('/ShowHotProducts', function (req, res) {	
	var q = 'select top 5 Productname from Products where LastSaleDate >= DATEADD(dd, -7, GETDATE()) order by LastSaleDate DESC';
...
app.get('/ShowNewProducts', function (req, res) {	
	var q = 'select Productname from Products where creationDate >= DATEADD(mm, -1, GETDATE()) order by CreationDate DESC';
	DButilsAzure.Select(q)    
		.then(function(ans) {
			res.send(ans);
			})
...
app.get('/GetAllProduct', function (req, res) {	
	var q = 'select Productname from Products';
	DButilsAzure.Select(q)    
		.then(function(ans) {
			res.send(ans);
			})
